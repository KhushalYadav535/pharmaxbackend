...
export const dailyReportController = {
  ...
  async getById(req: Request, res: Response) {
    try {
      const data = await dailyReportService.getById(req.params.id);
      res.json({ success: true, data });
    } catch (err: any) {
  ...
  async update(req: Request, res: Response) {
    try {
      const data = await dailyReportService.update(req.params.id, req.body);
      res.json({ success: true, data });
    } catch (err: any) {
      res.status(400).json({ success: false, message: err.message });
    }
  ...
  async delete(req: Request, res: Response) {
    try {
      await dailyReportService.delete(req.params.id);
